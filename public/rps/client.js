...
socket.on('updateRoomList', (roomList) => {
    ...
    for (const roomId in roomList) {
        ...
        if (room.gameType !== 'rps') continue; // 仅显示石头剪刀布的房间
...
document.getElementById('confirmSettings').addEventListener('click', () => {
    ...
    socket.emit('createRoom', 'rps', settings, (id) => {
        roomId = id;
        document.getElementById('roomId').innerText = `房间 ID: ${roomId}`;
...
document.getElementById('quickMatch').addEventListener('click', () => {
    socket.emit('quickMatch', 'rps', (roomIdReceived, settings) => {
...
function applyGameSettings(settings) {
    if (settings.victoryCondition === 'score') {
        document.getElementById('targetScoreLabel').style.display = 'inline';
        document.getElementById('targetScore').style.display = 'inline';
        document.getElementById('totalRoundsLabel').style.display = 'none';
        document.getElementById('totalRounds').style.display = 'none';
        ...
        document.getElementById('totalRoundsLabel').style.display = 'inline';
        document.getElementById('totalRounds').style.display = 'inline';
    }
    // 初始化当前回合数
